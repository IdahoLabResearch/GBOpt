...
def test_supercell_axis_numerators_rejects_negative_determinant_supercell():
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(3, ValueError, id="too-large"),
        pytest.param(1.5, TypeError, id="float"),
        pytest.param(True, TypeError, id="boolean"),
        pytest.param(np.bool_(False), TypeError, id="numpy-boolean"),
    ],
)
def test_supercell_axis_numerators_rejects_invalid_axis(axis, exception):
    with pytest.raises(exception, match="axis must be 0, 1, or 2"):
        supercell_axis_numerators(
            _int_matrix(IDENTITY_ROWS),
            [[0, 0, 0]],  # type: ignore[ty:invalid-argument-type]
            axis=axis,
